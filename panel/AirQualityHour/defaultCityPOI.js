...
POI_AirQualityHourCity = {
    ...
    searchPOI: function (params, callback) {
        ...
        $.ajax({
            type: "POST",
            url: config.service.hjjcService + "/GetCityDayQuaityData",
            data: JSON.stringify({ cityCode: '410100000', timePoint: time }),
    ...
    mapShowPOI: function (json, params) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            var id = json[i]["CityCode"];
            json[i]["p_id"] = id;
            json[i]["p_name"] = json[i]["CityName"];
            json[i]["p_lon"] = lonlat.lon || 0;
            json[i]["p_lat"] = lonlat.lat || 0;
            json[i]["p_value"] = value;
            json[i]["p_urlParam"] = "&time=" + monitorTime + "&code=" + id + "&pointType=" + "city" + "&jianceType=" + params.monitorType || "AQI";
            //其他必要属性etc
