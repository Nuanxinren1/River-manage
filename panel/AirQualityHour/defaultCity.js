...
var citySearchFunc = {
    ...
    */
    ...
    search: function (params, callback) {
        ...
        //查询点位实时监测值信息
        POI_AirQualityHourCity.searchPOI(params, function (json) {
            //test增加9条测试数据meilc
            ...
            //加载poi点位
            POI_AirQualityHourCity.mapShowPOI(json);
        });
    },
    ...
    tableShow: function (json) {
        ...
        for (var i = 0; i < json.length; i++) {
            if (true) {//checkInRegion(json[i]["REGION_CODE"])
                ...
                object["code"] = json[i]["CityCode"];
                object["name"] = json[i]["CityName"];
                ...
                var lonlat = mugis.mapCityInfo.getCityCoorByRegionCode(city_lon_lat, json[i]["CityCode"]);
                object["lon"] = lonlat.lon;
                object["lat"] = lonlat.lat;
                data.push(object);
            }
        }
        ...
        utils.tableInit.initTable("#airCityTable", null, data, columns, null, false, citySearchFunc.onClickRow);
    },
    ...
    onClickRow: function (row, ele) {
        ...
        //查询并加载单个点位信息
        citySearchFunc.searchOneData(row["code"]);
    ...
    loadStatiscsInfo_oneStatiscs: function (statiscsType) {
        ...
        var searchcode = this.oneJson["CityCode"];
