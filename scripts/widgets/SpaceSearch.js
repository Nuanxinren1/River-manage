    ...
    declare("widgets.SpaceSearch", null, {
        ...
        addGraphicToMap: function (evt) {
           ...
           $.getJSON("mock/company.json",function (result) {
            for(var z=0;z<result.list.length;z++){
                // debugger;
                var   point = new Point(result.list[z]["JD"], result.list[z]["WD"], map.spatialReference);
                if (geometry.contains(point)) {
                    pointsHcMsg.push(result.list[z]);
                }
             }
             mapShowPOIAddPointSZ(pointsHcMsg,"all");
             var  baseSearchMap={};
             load_info(pointsHcMsg,baseSearchMap,"");
             $(".count").html(pointsHcMsg.length);
           })
