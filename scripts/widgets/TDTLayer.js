        ...
        return declare([TiledMapServiceLayer], {
            ...
            getTileUrl: function (level, row, col) {
                ...
                if (this.noteType === "vec_c") {
                    tileUrl = this._url + "?tk=06e68c26a5ee7ed37a610d5d77d9ce12&SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=vec&STYLE=default&TILEMATRIXSET=c&TILEMATRIX=" + level + "&TILEROW=" + row + "&TILECOL=" + col + "&FORMAT=tiles";
                }
                else if (this.noteType === "cva_c") {
                    tileUrl = this._url + "?tk=06e68c26a5ee7ed37a610d5d77d9ce12&Service=WMTS&Request=GetTile&Version=1.0.0&Style=Default&Format=tiles&serviceMode=KVP&layer=cva&TileMatrixSet=c&TileMatrix=" + level + "&TileRow=" + row + "&TileCol=" + col;
                }
                else if (this.noteType === "ter_c") {
                    tileUrl = this._url + "?tk=06e68c26a5ee7ed37a610d5d77d9ce12&SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=ter&STYLE=default&TILEMATRIXSET=c&TILEMATRIX=" + level + "&TILEROW=" + row + "&TILECOL=" + col + "&FORMAT=tiles";
                }
                else {
                    tileUrl = this._url + "?tk=06e68c26a5ee7ed37a610d5d77d9ce12&SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&STYLE=default&TILEMATRIXSET=c&TILEMATRIX=" + level + "&TILEROW=" + row + "&TILECOL=" + col + "&FORMAT=tiles";
                }
                return tileUrl;
            }
        });
